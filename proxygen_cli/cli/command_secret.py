...
@click.option(
    "--env",
    type=CHOICE_OF_ENVS, help="Only print secrets in the choice environment."
)
def list(ctx, env):
    """
    List secrets.
...
def _validate_put_options(
        secret_value: str, secret_file: str, apikey: bool,
        mtls_cert: str, mtls_key: str
        ) -> Optional[str]:
    """
...
def _read_or_fail(secret_file: str, type_label: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"Missing or undreadable {type_label} "
                         f"file {secret_file}.") from ex
...
@click.option(
    "--mtls-cert",
    help="Provide a path to a PEM encoded certificate for mutual TLS"
)
@click.option(
    "--mtls-key",
    help="Provide a path to a PEM encoded private key for mutual TLS"
)
@click.pass_context
